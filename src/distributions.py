...
class _InterpolatedDistribution(rv_continuous):
    ...
    def _add_interpolants(self, n_average):
        self._cached_logpdf_interpolants[n_average] = interp1d(
            self._data["x"], self._data[str(n_average)],
            bounds_error=False, fill_value=0
        )
        cdf = cumtrapz(
            np.exp(self._cached_logpdf_interpolants[n_average](
                    np.abs(self.x_values)
            )),
            self.x_values,
            initial=0
        )
        ...
        self._cached_inverse_cdf_interpolants[n_average] = interp1d(
            cdf, self.x_values
        )
...
class MedianMarginalisedDistribution(_InterpolatedDistribution):    
    _data = pd.read_csv(
        os.path.join(__dir__, "whitening.dat"), sep="\t"
    )
